...
looic.df <- data.frame(model = out.names,
                       looic = loo.all) %>%
  arrange(by = "looic")
...
```{r}
jm <- readRDS(file = paste0("RData/JAGS_out_rSlope_rInt_", looic.df[1,"model"], "_FL.rds"))
...
```{r}
sum(loo.out[[1]]$loo.out$diagnostics$pareto_k > 0.7)/length(STX.jags.data$count)
```
