...
jags.data <- list(n.beaches = nrow(y),
                  n.years = summary.years$n.years[27:29],
                  y = y)
...
for (k in 1:length(NA.idx.2)){
  if (length(NA.idx.2[[k]]) > 0){
    for (k1 in 1:length(NA.idx.2[[k]])){
      missing.y <- c(missing.y, paste0("y[", k, ",", NA.idx.2[[k]][k1], "]"))
    }
  }
}
