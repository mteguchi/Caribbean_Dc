...
Pareto k statistic isn't great though... About `r round(high_pareto_k, 1) *100` % were > 0.7. Should we be concerned?  
...
for (b in 1:nrow(FL_beach_ID)){
  ...
  for (k in 0:(nrow(beach.data)-1)){
    t <- beach.data[(k+1), "Year"] - min(beach.data$Year) #k + min(beach.data$Year)
    if (t < median(change.point.samples)){
      tmp <- a0.1.samples + a1.1.samples * t + beta.1.samples * beach.data$X1[k+1] + 
        beta.2.samples * beach.data$X2[k+1] + beta.3.samples * beach.data$X3[k+1]
    } else {
      tmp <- a0.2.samples + a1.2.samples * t + beta.1.samples * beach.data$X1[k+1] + 
        beta.2.samples * beach.data$X2[k+1] + beta.3.samples * beach.data$X3[k+1]
    }
    
    line.df[k+1,] <- c(beach.data[(k+1), "Year"], mean(tmp), 
...
Some beaches don't look right... check what's going on. 2022-01-28
