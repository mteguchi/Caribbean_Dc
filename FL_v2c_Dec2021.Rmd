...
In v2a, I looked at site-specific change in slopes. It appeared that the year.change parameter was not converging well. Posteriors were quite diffuse over the parameter space. In v2b, I used a single year.change parameter for all locations. In this version (v2c), I use normal likelihood, rather than Poisson, as it was better when analyzing STX data. 

Some beaches don't have good data to be included - too many zeros, which return error in fitting the normal likelihood. If I remove some beaches, I won't be able to compare with the Poisson model outputs. So... I decided that the maximum count for each beach has to be at least 10 and run all models again. This restriction reduces the number of datasets from 68 to 35. 
...
# MCMC.params <- list(n.samples = 750,
#                     n.burnin = 550,
#                     n.thin = 5,
#                     n.chains = 5)
...
FL.nest.counts.0 %>% 
  select(ID2, ID, latitude, beach_f, year, nests) %>% 
  group_by(ID) %>%
...
FL.jags.data <- list(N = length(FL.nest.counts$nests),
                     ...
                     log.count = log(FL.nest.counts$nests + 1),
                     beach = median.yr$ID3,
...
for (k in 1:length(out.names)){
  MCMC.params$model.file = paste0("models/Model_JAGS_LogNorm_r2Slopes_rInt_",
                                  model.names[k], "_a.txt")
  FL.jags.data$X <- X.FL[[k]]

  if (!file.exists(paste0("RData/JAGS_out_LogNorm_r2Slopes_rInt_", 
    ...
    saveRDS(jm, 
            file = paste0("RData/JAGS_out_LogNorm_r2Slopes_rInt_", 
                          out.names[k], "_a_FL.rds"))
    
  } else {
    jm <- readRDS(file = paste0("RData/JAGS_out_LogNorm_r2Slopes_rInt_",
                                out.names[k], "_a_FL.rds"))
  }

  Rmax[k] <- max(unlist(lapply(jm$Rhat, FUN = max)))
  
  if (!file.exists(paste0("RData/LOOIC_LogNorm_r2Slopes_rInt_", 
                          out.names[k], "_a_FL.rds"))){
    loo.out[[k]] <- compute.LOOIC(loglik = jm$sims.list$loglik, 
                                  data.vector = FL.jags.data$log.count, 
                                  MCMC.params = MCMC.params)
    saveRDS(loo.out[[k]], 
            file = paste0("RData/LOOIC_LogNorm_r2Slopes_rInt_",
                          out.names[k], "_a_FL.rds"))
    
  } else {
    loo.out[[k]] <- readRDS(file = paste0("RData/LOOIC_LogNorm_r2Slopes_rInt_",
                                          out.names[k], "_a_FL.rds"))
  }
