---
title: "FL_v2_Dec2021; Site-specific 2 slope models for FL with Normal likelihood"
output: html_notebook
---

In v2a, I looked at site-specific change in slopes. It appeared that the year.change parameter was not converging well. Posteriors were quite diffuse over the parameter space. In v2b, I used a single year.change parameter for all locations. In this version (v2c), I use normal likelihood, rather than Poisson, as it was better when analyzing STX data. Here (v2d), I added more beta parameters - one set for before year.change and another for after. 
...
```{r}
mcmc_dens(jm$samples, c("beta[1,1]","beta[1,2]","beta[1,3]",
...
```{r}
 # beta parameters
beta.1.1.samples <- extract.samples("beta[1,1]", jm$samples)
beta.1.2.samples <- extract.samples("beta[1,2]", jm$samples)
beta.1.3.samples <- extract.samples("beta[1,3]", jm$samples)

beta.2.1.samples <- extract.samples("beta[2,1]", jm$samples)
beta.2.2.samples <- extract.samples("beta[2,2]", jm$samples)
beta.2.3.samples <- extract.samples("beta[2,3]", jm$samples)

betas.df <- data.frame(samples = c(beta.1.1.samples, beta.1.2.samples, beta.1.3.samples,
                                   beta.2.1.samples, beta.2.2.samples, beta.2.3.samples),
                       pars = c(rep("beta.1.1", times = length(beta.1.1.samples)),
                                rep("beta.1.2", times = length(beta.1.2.samples)),
                                rep("beta.1.3", times = length(beta.1.3.samples)),
                                rep("beta.2.1", times = length(beta.2.1.samples)),
                                rep("beta.2.2", times = length(beta.2.2.samples)),
                                rep("beta.2.3", times = length(beta.2.3.samples))))

p.betas.density <- ggplot(data = betas.df) +
  geom_density(aes(x = samples), color = "darkblue") +
  facet_wrap(vars(pars), nrow = 2, scales = "free") +
  xlab("") + 
  ylab("Density") 

if (save.fig)
    ggsave(p.betas.density,
           filename = paste0("figures/Poi_a_betas_density.png"),
           device = "png", dpi = 600)
```
...
for (b in 1:nrow(FL_beach_ID)){
  ...
  for (k in 0:(nrow(beach.data)-1)){
    ...
    if (t < median(change.point.samples)){
      tmp <- exp(a0.1.samples + a1.1.samples * t + beta.1.1.samples * beach.data$X1[k+1] + 
        beta.1.2.samples * beach.data$X2[k+1] + beta.1.3.samples * beach.data$X3[k+1])
      
    } else {
      tmp <- exp(a0.2.samples + a1.2.samples * t + beta.2.1.samples * beach.data$X1[k+1] + 
        beta.2.2.samples * beach.data$X2[k+1] + beta.2.3.samples * beach.data$X3[k+1])
    }
  ...
  if (save.fig)
    ggsave(p.estimates,
           filename = paste0("figures/Poi_a_estimates_beach_", ID, ".png"),
           device = "png", dpi = 600)
  ...
  if (save.fig)
    ggsave(p.slope2.density,
           filename = paste0("figures/Poi_a_slopes_density_", ID, ".png"),
           device = "png", dpi = 600)
...
```{r}
loo.out.2slopes.2betas <- readRDS(file = "RData/LOOIC_r2Slopes_rInt_3Covs_a_FL.rds")
...
jm.2slopes <- readRDS(file = "RData/JAGS_out_r2Slopes_rInt_3Covs_FL.rds")

loo.final.df <- data.frame(slope = c("1slope", "2slopes", "2slopes_2betas"),
                           model = c("logD", "3Covs", "3Covs"),
                           looic = c(loo.out.1slope$loo.out$estimates["looic", "Estimate"],
                                     loo.out.2slopes$loo.out$estimates["looic", "Estimate"],
                                     loo.out.2slopes.2betas$loo.out$estimates["looic", "Estimate"]),
                           DIC = c(jm.1slope$DIC, jm$DIC, jm.2slopes$DIC)) %>%
...
So, it appears that there is evidence for using two-slope-two-beta models. 

Still working on normal likelihood models... 2022-02-03
