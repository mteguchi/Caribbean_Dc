...
The observed nest counts at St Croix indicated a continuous increase in the numbers until 2001, somewhat steady counts until 2009, and a  continuous decline since then (Figure \@ref(fig:plot-raw)). We therefore fitted models that can accommodate such changes in nest counts. Nest counts were considered to be distributed as either Poisson or log-Normal distribution. We also considered using the number of survey days per year as a covariate for all model sets. 
...
For this set of models, we used the Poisson distribution as the likelihood function of the observed nest counts ($y_i$). We treated the change point ($15 < \omega < 30$) to be an additional parameter to be estimated.   

$y_i \sim POI(\mu_i)$

$\mu_i = \beta_{0,1} + \beta_{1,1} \times X_i + \beta_{2,1} \times T + \epsilon_i$ if $T \lt \omega$ 

$\mu_i = \beta_{0,2} + \beta_{1,2} \times X_i + \beta_{2,2} \times T + \epsilon_i$ if $T \ge \omega$ 

$X_i$ is the number of survey days for the i-th year. 
...
MCMC converged for both models where Rhat statistic was less than 1.1. LOOIC indicated that both models are virtually equivalent (\@ref(tab:table-Poi)). Consequently, we used the simpler model with no covariate as the best model. Pareto k statistics indicated that the model did not fit to a large proportion of the data points (p = `r prop.high.pareto.k.Poi`).
...
The second set of models used the normal distribution as the likelihood function for log-transformed nest counts ($y_i$). Similarly to the Poisson models, we treated the change point ($15 < \omega < 30$) as an unknown parameter. 
...
$\sigma_i = \sigma_1$ if $T \lt \omega$

$\sigma_i = \sigma_2$ if $T \ge \omega$
...
MCMC converged for both models where Rhat statistic was less than 1.1. LOOIC indicated that both models are virtually equivalent (\@ref(tab:table-Normal)). Consequently, we used the simpler model with no covariate as the best model. Pareto k statistics indicated that the model fit well to a large proportion of the data points (p = `r 1 - prop.high.pareto.k.Normal`).
...
knitr::include_graphics("figures/STX_change_yr_hist.png")
...
```{r plot-growth-rate, echo=FALSE, cache=TRUE, fig.cap = "The marginal posterior distributions annual growth rates."}
